...
def train_model(cfg,
    ...
    best = 0.
    writer_train = LogWriter(logdir=f"./log/{cfg.model_name}/train")
    writer_valid = LogWriter(logdir=f"./log/{cfg.model_name}/vaild")
    for epoch in range(0, cfg.epochs):
        ...
        tic = time.time()
        for i, data in enumerate(train_loader): ###data:[x,y]
        ...
        ##################train log######################
        for m in record_list:#record_list:{name:AverageMeter}
            if not (m == 'batch_time' or m == 'reader_time'):
                writer_train.add_scalar(tag=m, value=record_list[m].avg,step=epoch)
        ###################################################
        ...
        def evaluate(best):
            ...
            for m in record_list:
                if not (m == 'batch_time' or m == 'reader_time'):
                    writer_valid.add_scalar(tag=m, value=record_list[m].avg,step=epoch)
            ###########################################################
            ...
            if best_flag:
                np.save(osp.join(output_dir, model_name + "_best_score"),score_list)
