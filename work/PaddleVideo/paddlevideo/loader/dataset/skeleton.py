...
class SkeletonDataset(BaseDataset):
    ...
    """
    def __init__(self, file_path, pipeline, label_path=None, test_mode=False, weight_path=None):
    ...
    def load_file(self):
        ...
        if self.weight_path:
            self.weight = np.load(self.weight_path)
    ...
    def prepare_train(self, idx):
        ...
        if self.weight_path:
            weight = copy.deepcopy(self.weight[idx])
            return results['data'], results['label'], weight
        else:
