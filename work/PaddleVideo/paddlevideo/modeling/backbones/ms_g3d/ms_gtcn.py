...
class UnfoldTemporalWindows(nn.Layer):
    ...
    def forward(self, x):
        ...
        x = x.reshape((N, C, self.window_size, -1, V)).transpose((0,1,3,2,4))
...
class SpatialTemporal_MS_GCN(nn.Layer):
    def __init__(self,
        ...
        self.A_scales = paddle.to_tensor(A_scales)
        self.V = len(A_binary)

        if use_Ares:
            self.A_res = self.create_parameter(
                    shape=self.A_scales.shape,
                    default_initializer=nn.initializer.Uniform(low=-1e-6, high=1e-6))
            # self.A_res = weight_init_(nn.Parameter(paddle.randn(self.A_scales.shape)),'Uniform',low=-1e-6, high=1e-6)
            # self.A_res = nn.init.uniform_(nn.Parameter(paddle.randn(self.A_scales.shape)), -1e-6, 1e-6)
        else:
            self.A_res = paddle.to_tensor(0)
    ...
    def forward(self, x):
        ...
        # Build graphs
        A = self.A_scales.astype(x.dtype) + self.A_res.astype(x.dtype)
        # A = self.A_scales.to(x.dtype).to(x.device) + self.A_res.to(x.dtype).to(x.device)

        # Perform Graph Convolution
        res = self.residual(x)
        agg = einsum(x,A)
        agg = agg.reshape((N, C, T, self.num_scales, V))
        agg = agg.transpose((0,3,1,2,4)).reshape((N, self.num_scales*C, T, V))
