...
class ResStage(paddle.nn.Layer):
    ...
    def forward(self, inputs):
        output = []
        for pathway in range(self.num_pathways): # only one path
            x = inputs
            for i in range(self.num_blocks[pathway]):
...
class VideoModelStem(paddle.nn.Layer):
    ...
    def forward(self, x):
        ...
        for pathway in range(1): # only one path
...
class ResNetSlowOnly(paddle.nn.Layer):
    ...
    def _construct_network(self):
        ...
        dim_inner = self.width_per_group * self.num_groups  #32

        self.s3 = ResStage(dim_in=[self.width_per_group],#32
                           dim_out=[self.width_per_group * 4],#128
        ...
        self.s4 = ResStage(
            dim_in=[self.width_per_group * 4],#128
            dim_out=[self.width_per_group * 8],#256
            dim_inner=[dim_inner * 2],#64
        ...
        self.s5 = ResStage(
            dim_in=[self.width_per_group * 8],#256
            dim_out=[self.width_per_group * 16],#512
            dim_inner=[dim_inner * 4],#128
